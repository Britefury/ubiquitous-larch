...
def code_mirror(text, immediate_events=False, config=None, on_edit=None, on_focus=None, on_blur=None, modes=None):
	...
	p = p.use_css('/codemirror-3.14/lib/codemirror.css')
	p = p.use_js('/codemirror-3.14/lib/codemirror.js')
	for mode in modes:
		p = p.use_js('/codemirror-3.14/mode/{0}/{0}.js'.format(mode))
	for addon in __addon_js:
		p = p.use_js('/codemirror-3.14/addon/{0}'.format(addon))
	for addon in __addon_css:
		p = p.use_css('/codemirror-3.14/addon/{0}'.format(addon))
	if __code_mirror_theme is not None:
		p = p.use_css('/codemirror-3.14/theme/{0}.css'.format(__code_mirror_theme))
